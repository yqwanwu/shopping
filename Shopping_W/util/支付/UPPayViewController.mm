...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    {
        if ([tn hasPrefix:@"\""]) {
            tn = [tn substringWithRange:NSMakeRange(1, tn.length - 2)];
        }
        
        [[UPPaymentControl defaultControl] startPay:tn fromScheme:@"caizhu" mode:kMode_Development viewController:_topVc];
...
+ (void)startPayWithTopVc:(UIViewController *)topVc id:(int)oid isOrderList:(BOOL)isOrderList {
    UPPayViewController *vc = [[UPPayViewController alloc] init];
    vc.topVc = topVc;
    
    NSString *url = [[NSString stringWithFormat:@"%@person/memberService.d2js?_m=PayOrder4UnionPay&params={\"id\":%d, \"isOrderList\":%@}", host, oid, isOrderList ? @"true" : @"false"]  stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    [vc startNetWithURL:[NSURL URLWithString:url]];
}
